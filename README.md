...
- 需求：脚本面向**新创建的用户**，运行会覆盖/修改已有配置；运行时先提示将改动哪些配置，仅作提醒、不拦截。
- 实现：打印即将覆盖的配置清单（~/.vimrc、~/.screenrc、~/.ssh/config、~/.sh_profile、~/.zshrc、~/.bashrc、~/.tmux.conf），按任意键继续（Ctrl+C 可中断）。
